...
- **Traditional TTS**: Selecting the “Amy (en-US)” preset copies the bundled Piper `.onnx`/`.json` pair into `ApplicationSupportDirectory/voices/amy-medium`. The Rust core loads that file, feeds it through `piper-rs` + ONNX Runtime, and streams the PCM back to Flutter along with per-chunk indices for word highlighting.
